...
#Error handling for image encoding process
...
#Function to resize image to 4:3 ratio
...
#Function to extract image files from Google Drive URL
...
# Function to change file name to proper format
def sanitize_filename(name):
    name = name.replace(' ', '-')
    return re.sub(r'[/\\:*?"<>|]', '', name)

# Function to remove deleted images from the local directory
...
# Function to download images from Google Sheets
def download_img(current_directory,images_directory,JSON_FILENAME, SPREADSHEET_URL, SHEET_NAME, scope):
    global worksheet
